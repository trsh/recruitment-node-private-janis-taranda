...
export class AuthService {
  ...
  public async checkJwtToken(tokenStr: string): Promise<AccessToken | null> {
    const results = await this.accessTokenRepository.find({
      where: {
        token: tokenStr
      },
    ...
    if (results?.length === 1) {
      ...
      const tokenExpireDate = this.getJwtTokenExpireDate(token.token);

      if (tokenExpireDate < this.unixTimestamp()) {
        return null;
      }
  ...
  private unixTimestamp(date = Date.now()): number {
